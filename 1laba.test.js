...
describe("calculateSubscriptionEndDate — расчёт даты окончания подписки", () => {
  ...
  describe("некорректная длительность подписки", () => {
    test("ошибка при неподдерживаемой длительности", () => {
      expect(() => calculateSubscriptionEndDate("10 марта 2024", 2)).toThrow(
        "Недопустимая длительность подписки",
      );
    });

    test("ошибка при отрицательной длительности", () => {
      expect(() => calculateSubscriptionEndDate("10 марта 2024", -1)).toThrow(
        "Недопустимая длительность подписки",
      );
    });
  });
  ...
  describe("некорректный формат даты", () => {
    test("ошибка при неверном формате", () => {
      expect(() => calculateSubscriptionEndDate("2024-03-10", 1)).toThrow(
        "Неверный формат даты",
      );
    });

    test("ошибка при отсутствии частей даты", () => {
      expect(() => calculateSubscriptionEndDate("10 марта", 1)).toThrow(
        "Неверный формат даты",
      );
    });
  });
  ...
  describe("некорректные значения даты", () => {
    test("31 февраля — недопустимая дата", () => {
      expect(() => calculateSubscriptionEndDate("31 февраля 2024", 1)).toThrow(
        "Некорректная дата начала",
      );
    });

    test("день равен нулю", () => {
      expect(() => calculateSubscriptionEndDate("0 марта 2024", 1)).toThrow(
        "Некорректные значения даты",
      );
    });

    test("неизвестное название месяца", () => {
      expect(() => calculateSubscriptionEndDate("10 мартабля 2024", 1)).toThrow(
        "Некорректные значения даты",
      );
    });
...
describe("успешный сценарий — оплата прошла, дата считается", () => {
  test("SUCCESS → должен вызвать calculateSubscriptionEndDate и вернуть её результат", async () => {
    ...
    const result = await calculateSubscriptionWithPayment(
      999,
      startDate,
      months,
    );
...
describe("сложный сценарий — расчёт даты НЕ выполняется", () => {
  ...
  beforeEach(() => {
    spy = jest.spyOn(require("./1laba"), "calculateSubscriptionEndDate");
  });
